...
// $.get('/whldt?month=' + month, function (data, status) {
$.get('/whldt', function (data, status) {
    var loupanInfo = data;
...
// collected markers
    ...
    loadAllWorldHesitages(loupanInfo);
...
// 默认加载的楼盘的标记
function loadAllWorldHesitages(whInfo) {
    // console.log("whInfo.loupans.length: " + whInfo.loupans.length);
    ...
    whInfo.forEach(function (data) {
        // console.log(data);
        var lpName = "<b>楼盘: </b> " + data.name + "<br>";
        if(data.hasOwnProperty('wenzurl')) { // 如果有微信文章链接
            lpName = "<b>Name: </b><a href=' " + data.wenzurl + "'> " + data.name + "</a><br>";
        }
        else  {  //如果没有微信页面链接
        ...
        var popCnt = "<div style='width: 100%;'>" +
            data.description +
            // lpType +
            "<div class='row'>"+
            "<div class=\"col-lg-6\">" +
        ...
        console.log("pop content: " + popCnt);
        // if(data.taoshu !== '') {
        ...
        var marker1 = L.marker(data.coordinate_digit, {opacity: 0.5})
                        .bindPopup(popCnt)
                        .openPopup();
        var circle = L.circle(data.coordinate_digit, {
            color: 'red',
            fillColor: null,
            fillOpacity: 0.5,
            radius: 10
