...
func main() {
	...
	getIF := func() (tif net.Interface) {
		for _, iface := range ifaces {
			addrs, _ := iface.Addrs()
			for i := range addrs {
				if strings.HasPrefix(addrs[i].String(), "10.0.0.") {
					Lg.Debugf("%v, name: %v", addrs[i].String(), iface.Name)
					tif = iface
				}
			}
		}
	...
	targetIFace := getIF()
	if err := scan(&targetIFace); err != nil {
		Lg.Debugf("interface %v: %v", targetIFace.Name, err)
	}
}
...
func readARP(in chan gopacket.Packet, iface *net.Interface, stop chan struct{}) {
